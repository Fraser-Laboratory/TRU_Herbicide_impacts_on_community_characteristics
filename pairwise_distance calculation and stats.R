...
m_braydist <- lmer((mean_val) ~ rowname_time * sprayed + (1|name), 
                   dist_from_baseline) 
DHARMa::simulateResiduals(m_braydist, plot = T)
car::Anova(m_braydist, test.statistic = "F")
  ggplot(dist_from_baseline, aes(x = sprayed, y = mean_val)) +
  stat_summary(geom = "pointrange", fun.data = "mean_sdl", linewidth = 1.5, size = 1.5, color = "steelblue") +
